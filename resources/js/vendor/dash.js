...
class DashResource {
	constructor(resource) {
		if (Dash.config.baseResourceURL != null) {
			...
			console.warn(
				new DashConfigException("baseResourceURL config not set")
			);
		}
	}
...
var Dash = {
	...
	url: function (concat) {
		...
		else throw "Base URL not set in config";
	},
	...
	getView: function (url, configuration = {}) {
		...
		if (typeof url == "number") {
			...
			this.views.forEach(function (node) {
				if (node.getAttribute("data-name") == url) viewAnchor = node;
			});
		}
				...
				Dash.renderView(response, config.container)
					.then((response) => {
						...
						let viewName = viewAnchor
							? viewAnchor.getAttribute("data-name")
							: null;
						...
						if (typeof Dash.getView.then === "function") {
							...
							if (config.defaultCallback) {
								Dash.getView.then.call(
									this,
									viewName,
									response,
									Dash.select(Dash.config.content)
								);
							}
						}
	...
	bind: function (
		...
		if (element) {
			element.addEventListener(
				...
				function (e) {
					var target = e.target,
						...
						childSelectors = selector.replace(regex, function (
							match
						) {
							return " *,";
						});

					var matchesChild = target.matches(
						childSelectors.trim() + " *"
					);
			...
			throw new DashSelectorException(
				parentSelector + " does not any element in the DOM"
			);
		}
	},
	...
	init: function (globalConfig = {}) {
		...
		this.views.forEach(function (element, index) {
			element.addEventListener(
				"click",
				Dash.handlers.handleViewAnchorClick,
				false
			);
		});
		...
		if (this.config.triggerView) {
			try {
				...
				if (e.name == "TypeError") {
					// Log exception
					console.warn(
						new DashSelectorException(
							"View selector does not match any element"
						)
					);
				}
			}
	...
	bindData: function (bindingFrom, bindingTo) {
		...
		bindables.forEach(function (node) {
			var name = node.getAttribute("data-name"),
				toElements = bindingTo.querySelectorAll(
					"[data-name=" + name + "]"
				);
	...
	request: function request(url, data, successfn, configuration = {}) {
		...
		xhr.onreadystatechange = function () {
			switch (xhr.readyState) {
				...
				case XMLHttpRequest.DONE:
					...
					if (
						!isViewRequest &&
						typeof Dash.request.then === "function"
					)
						Dash.request.then(response);
		...
		xhr.onprogress = function (e) {
			...
			// Call default progress handler
				...
				!isViewRequest &&
				typeof dash.request.progress === "function"
			)
	...
	form: {
		...
		sanitize: function (form) {
			var inputs = (
				form || document.querySelector(Dash.config.activeForm)
			).querySelectorAll(
		...
		checkEmptyInputs: function (nodes) {
			...
			for (let i = 0; i < nodes.length; i++) {
				if (nodes[i]) {
					...
					throw (
						"Required input element " + index + " supplied is null"
					);
				}
			}
		...
		create: function (uri, formdata, callback, config) {
			...
			var url = uri || Dash.getActiveForm().action,
				fn = function (response) {
					// this -> XMLHttpRequest
					Dash.resources.defaultCallback.call(
						this,
						callback,
						response,
						config
			...
			if (config.cache && search.results[q] != null) {
				...
				resultElements = containerElements.filter(function (container) {
					try {
						...
						console.error(error);
						throw new DashTypeException(
							"[data-name] attribute not found on element",
							container
						);
					}
				});
	...
	history: {
		...
		init: function () {
			window.addEventListener("popstate", function (e) {
				...
				if (
					typeof Dash.history.popActions[Dash.history.current] ==
					"function"
				) {
					Dash.history.popActions[Dash.history.current].call(
						e,
						e.state
					);
				}
